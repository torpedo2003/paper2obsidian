...
def _detect_bibliography_setup(work_dir, main_rel):
    ...
    # Prefer reusing an existing .bbl when available; this avoids relying on
    # remote bibliography tooling whose API varies across compiler backends.
    if prebuilt_bbl:
...
def _detect_compiler(work_dir, main_rel):
    ...
    if "\\usepackage{xeCJK}" in main_text or "\\setCJKmainfont" in main_text:
        return "xelatex"
    if (
        "\\usepackage{luatexja}" in main_text
        or "\\usepackage{luatexja-fontspec}" in main_text
        or "\\setmainjfont" in main_text
        or _CTEX_PACKAGE_RE.search(main_text)
    ):
        return "lualatex"
...
def _ensure_cjk_support(work_dir, main_rel):
    ...
    ) or _CTEX_PACKAGE_RE.search(text):
        return False
...
def _preflight_comment_inputenc_fontenc(work_dir, main_rel):
    ...
    uses_unicode_stack = _source_uses_unicode_stack(source_texts.values())
    ...
    for rel, text in source_texts.items():
        ...
        for pat in (
            ...
            new_text = re.sub(pat, _comment_line, new_text, count=1, flags=re.MULTILINE)
        if new_text == text:
            continue
        try:
            with open(path, "w", encoding="utf-8") as f:
...
def _preflight_comment_pdfoutput(work_dir, main_rel):
    # LuaLaTeX/XeLaTeX do not need the pdfTeX-specific \pdfoutput switch and
    # some builds treat it as an undefined control sequence.
    path = os.path.join(work_dir, main_rel)
    try:
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            text = f.read()
    ...
    pat = r"^(?P<indent>\s*)\\pdfoutput\s*=\s*\d+\s*$"
    ...
    new_text = re.sub(pat, _comment_line, text, count=1, flags=re.MULTILINE)
    if new_text == text:
        return False
