...
_ARXIV_ID_RE = re.compile(r"(?P<id>\d{4}\.\d{4,5}(?:v\d+)?|[A-Za-z-]+(?:\.[A-Z]{2})?/\d{7}(?:v\d+)?)")


def normalize_paper_id(raw):
    """Accept arXiv IDs, abs/e-print URLs, and common pasted variants."""
    s = str(raw).strip()
    if not s:
        return s
    ...
    if parsed.scheme and parsed.netloc:
        ...
        if path.endswith(".pdf"):
            path = path[:-4]
        s = path
    match = _ARXIV_ID_RE.search(s)
    return match.group("id") if match else s
...
def extract_tar_archive(tf, work_dir):
    ...
    if sys.version_info >= (3, 12):
        ...
        for member in tf.getmembers():
            ...
            if not _is_within_directory(work_dir, target):
                continue
            # Avoid link traversal on older Python runtimes.
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        sys.exit(2)
    paper_id, work_dir = normalize_paper_id(sys.argv[1]), sys.argv[2]
